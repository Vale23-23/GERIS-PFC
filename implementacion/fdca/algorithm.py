...
def run_fdca(inp: FDCAInput) -> FDCAOutput:
    ...
    fire_mask, fail_char_arr, candidates = run_part1(
        ...
        region_mask=(inp.region_mask if inp.region_mask is not None else np.ones_like(inp.land_mask, dtype=bool)),
        eco_mask=(inp.eco_mask if inp.eco_mask is not None else np.zeros_like(inp.land_mask, dtype=np.uint8)),
