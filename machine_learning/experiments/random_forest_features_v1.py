"""Train a reproducible Random Forest on per-scene feature CSVs.

Random Forest counterpart of decision_tree_features_v1.py: same feature contract
emitted by build_features.py, same disjoint train/test scenes and the same
pixel- and scene-level metrics. Only the estimator changes.
"""
...
DEFAULT_EXPERIMENT_CONFIG = PROJECT_ROOT / "machine_learning" / "configs" / "experiments" / "rf_features_baseline_v1.yaml"
# If the RF config does not exist yet, reuse only the data/split settings of the DT baseline
# (never its hyperparameters, output_dir or experiment_name).
FALLBACK_EXPERIMENT_CONFIG = PROJECT_ROOT / "machine_learning" / "configs" / "experiments" / "dt_features_baseline_v1.yaml"
INHERITED_CONFIG_KEYS = {
    "features_dir", "feature_sets_config", "feature_set", "target", "split_dir", "split_id",
    "train_test_split", "scene_recall_target", "random_state",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Random Forest reproducible sobre CSVs de features por escena.")
    ...
    parser.add_argument("--n-estimators", type=int, default=None, help="Cantidad de árboles (default 300).")
    parser.add_argument("--max-depth", type=int, default=None, help="Profundidad máxima; sin límite si se omite.")
    ...
    parser.add_argument("--class-weight", choices=("balanced", "balanced_subsample", "none"), default=None)
    parser.add_argument("--ccp-alpha", type=float, default=None)
    parser.add_argument("--max-features", default=None, help="Número, fracción (0-1), sqrt, log2 o none (default sqrt).")
    ...
    parser.add_argument("--max-samples", default=None,
                        help="Filas (int) o fracción (float) que ve cada árbol; requiere bootstrap.")
    parser.add_argument("--no-bootstrap", action="store_true", help="Cada árbol usa todo el train (sin bootstrap).")
    parser.add_argument("--n-jobs", type=int, default=None, help="Núcleos para fit/predict (default -1 = todos).")
...
def load_experiment_config(path: str | None) -> dict[str, Any]:
    ...
    if resolved != DEFAULT_EXPERIMENT_CONFIG.resolve():
        raise FileNotFoundError(f"No existe --experiment-config: {resolved}")
    if FALLBACK_EXPERIMENT_CONFIG.exists():
        print(f"[aviso] No existe {resolved.name}; heredo sólo datos/split de {FALLBACK_EXPERIMENT_CONFIG.name}.",
              file=sys.stderr)
        legacy = load_yaml(FALLBACK_EXPERIMENT_CONFIG)
        return {key: value for key, value in legacy.items() if key in INHERITED_CONFIG_KEYS}
    print(f"[aviso] No existe {resolved.name}; uso sólo defaults y flags de CLI.", file=sys.stderr)
    return {}
...
def apply_config(args: argparse.Namespace, config: dict[str, Any]) -> None:
    mapping = {
        ...
        "n_estimators": "n_estimators", "max_samples": "max_samples", "n_jobs": "n_jobs",
    }
    ...
    args.output_dir = args.output_dir or str(PROJECT_ROOT / "machine_learning" / "outputs" / "random_forest")
    args.random_state = 42 if args.random_state is None else args.random_state
    args.n_estimators = 300 if args.n_estimators is None else args.n_estimators
    # max_depth=None (unlimited depth) is the usual Random Forest setting.
    ...
    args.max_features = "sqrt" if args.max_features is None else args.max_features
    args.n_jobs = -1 if args.n_jobs is None else args.n_jobs
    if args.n_estimators < 1:
        raise ValueError("--n-estimators debe ser >= 1")
    if args.no_bootstrap and args.max_samples is not None:
        raise ValueError("--max-samples requiere bootstrap (quitá --no-bootstrap).")
...
def parse_number(value: Any, name: str) -> int | float:
    raw = str(value).strip()
    try:
        return int(raw)
    except ValueError:
...
def parse_max_features(value: Any) -> int | float | str | None:
    raw = str(value).strip().lower()
    if raw == "none":
        return None
    if raw in ("sqrt", "log2"):
        return raw
    return parse_number(value, "max_features")


def hyperparameter_dict(args: argparse.Namespace) -> dict[str, Any]:
    """Hyperparameters that define the model (n_jobs is excluded: it does not change results)."""
    return {
...
def build_pipeline(features: list[str], args: argparse.Namespace) -> Pipeline:
    model = RandomForestClassifier(
        ...
        min_impurity_decrease=args.min_impurity_decrease,
        bootstrap=not args.no_bootstrap,
        max_samples=None if args.max_samples is None else parse_number(args.max_samples, "max_samples"),
        n_jobs=args.n_jobs,
        random_state=args.random_state,
    )
...
def main() -> None:
    ...
    run_spec = {
        ...
        "hyperparameters": hyperparameter_dict(args),
    }
    ...
    prefix = "" if label.startswith("rf_") else "rf_"
    run_id = f"{prefix}{label}_{split_metadata.get('split_id', 'split')}_{spec_hash}"
    ...
    try:
        ...
        # `test` was built with ignore_index=True, so group.index are row positions into
        # y_pred / y_probability: slice them instead of re-predicting every scene (costly for RF).
        for timestamp, group in test.groupby("timestamp", sort=True):
            rows = group.index.to_numpy()
            per_scene[timestamp] = scene_scores(y_test.iloc[rows], y_pred[rows], y_probability[rows])
        ...
        resolved = {
            ...
            "hyperparameters": {**hyperparameter_dict(args), "n_jobs": args.n_jobs},
            "environment": {
                "python": sys.version,
        ...
        tracker.log_parameters({
            ...
            "bootstrap": not args.no_bootstrap,
            "max_samples": "none" if args.max_samples is None else str(args.max_samples),
